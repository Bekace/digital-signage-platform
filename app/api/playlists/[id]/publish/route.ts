...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const result = await query("UPDATE playlists SET status = 'active', updated_at = NOW() WHERE id = $1 RETURNING *", [
      playlistId,
    ])
    ...
    return NextResponse.json({ error: "Failed to publish playlist" }, { status: 500 })
  }
}

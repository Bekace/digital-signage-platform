...
export async function POST(request: Request, { params }: { params: { playlistId: string } }) {
  ...
  try {
    ...
    const { media_file_id, duration, transition_type = "fade", position } = body
    ...
    const mediaFile = await sql`
      SELECT id, filename, file_size FROM media_files 
    ...
    if (itemPosition === undefined || itemPosition === null) {
      ...
      `
      itemPosition = (maxPosition[0]?.max_pos || -1) + 1
    }

    // Add item to playlist
    const newItem = await sql`
      INSERT INTO playlist_items (
        playlist_id, media_file_id, position, duration, transition_type
      )
      VALUES (
        ${playlistId}, ${media_file_id}, ${itemPosition}, ${duration || null}, ${transition_type}
      )
      RETURNING *
    `
    ...
    console.log(`✅ [PLAYLIST ITEMS API] Added item ${item.id} to playlist ${playlistId}`)

  ...
  try {
    ...
    const devices = await sql`
      SELECT 
        ...
        (SELECT COUNT(*) FROM playlist_items pi WHERE pi.playlist_id = p.id) as playlist_item_count
      FROM devices d
    ...
    // Format devices for response with correct field names
...
export async function POST(request: NextRequest) {
  try {
    ...
    const deviceResult = await sql`
      ...
      VALUES (${user.id}, ${name}, 'unknown', 'offline', 'none', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
      RETURNING *
    `
